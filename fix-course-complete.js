...
async function fixCourseStructure() {
  ...
  try {
    ...
    // Replace the entire curriculum structure
    const updateDoc = {
      $set: {
        curriculum: {
          modules: [
    ...
    if (result.matchedCount > 0) {
      console.log('✅ Course curriculum structure fixed successfully!');
      console.log('Modified count:', result.modifiedCount);
      
      // Verify the update
      const updatedCourse = await courses.findOne({ _id: courseId });
      console.log('\nUpdated curriculum structure:');
      console.log('Curriculum type:', typeof updatedCourse.curriculum);
      console.log('Has modules:', !!updatedCourse.curriculum.modules);
